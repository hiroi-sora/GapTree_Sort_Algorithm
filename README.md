...
简而言之：它根据每一整行中的文本块**间隙**，提取页面中所有的**竖切线**，将页面切割为不同的**区块**，将区块组织为**布局树**。最后，前序遍历布局树，即可得到符合人类阅读习惯的**文本块排序**。
...
部分情况下，布局树的节点可能位置不正确。比如在节点B寻找父节点时，误没有匹配到应有的父节点A，而匹配到父节点的父节点F（因为匹配规则是向上兼容的）。如下方所示。
...
```
    ...
    A                  |   A     B
 ↙                    |
B   正确匹配到父节点A   |  错误，匹配到爷节点F
```

但是，前序遍历时，节点总体的输出的顺序是正确的。与树的性质有关：上述两棵树的遍历顺序相同，都是F→A→B，因此B挂到A下 或挂到F下 是不会改变顺序的。

这个性质也为本算法提供了很强的鲁棒性。即使OCR文本块划分出错，比方说没有识别到破折号，而将一行文本错误的划分为两行。这会导致布局树构建出错，原本的一个区块被划分为2 ~ 4个。但是这不会影响输出顺序，因为划分后的2 ~ 4个区块，在前序遍历中是紧密相连的，不会被别的区块所隔开。从顺序上看，划分后的2 ~ 4个区块依然是“1个区块”。
...
在遍历行的循环之内，还有遍历当前间隙的内层循环。

假设所有间隙数量为k（k<n）。不管内层循环嵌套了多少层，内层总共的考察次数为k，因此内层循环对复杂度不构成影响（不是一个固定的乘法因子）。总体复杂度依然是 O(n) 。
...
但是对于常见布局，我们可以认为p是常数。比如对于标准双栏布局，不管n多大，布局树中都只有根节点+左右2个子节点。因此平均复杂度为 O(1) 。
